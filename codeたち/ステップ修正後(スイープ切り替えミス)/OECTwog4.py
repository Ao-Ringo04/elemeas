...
class MeasurementController:
    ...
    def _check_vref_reached_with_overshoot(self, target_vref: float, initial_vref: float, 
        ...
        """
        VrefがＧ標値に到達、または超過したかを判定（90%以上のサンプルが条件を満たしたら折り返す）
        
        【修正】初期値と目標値の大小関係で正確に方向判定
        
        折り返し条件（90%以上のサンプルが満たしたら折り返す）：
        
        【条件1：超過検出】
        - target_vref > initial_vref（上昇方向）: Vref >= target_vref で判定
        - target_vref < initial_vref（下降方向）: Vref <= target_vref で判定
        
        【条件2：許容範囲到達】
        - VrefがtargetVrefの±tolerance_percent以内で判定
        
        超過検出と許容範囲のいずれかを満たすサンプルが90%以上なら折り返す
        
        Args:
            target_vref: 目標Vref値 (V)
            initial_vref: 初期Vref値 (V) → 方向判定用、重要
            wait_time: 待機時間 (s)
            data_interval: データ取得間隔 (s)
            tolerance_percent: 許容誤差（パーセント）デフォルト 0.5%
            threshold_percent: 判定閾値（パーセント）デフォルト 90% → 新規
        ...
        # 【修正】方向を判定：初期値から目標値へ向かう方向
        # → target_vrefとinitial_vrefを比較（0ではなく）
        ...
        vref_samples = []
        condition_met_count = 0  # 条件を満たすサンプル数をカウント
        ...
        if percentage >= threshold_percent:
            ...
            vref_avg = sum(vref_samples) / len(vref_samples)
            print(f"    平均 Vref: {vref_avg:+.6f}V")
        else:
            print(f"    → {threshold_percent}%未満のため継続します ✗")
            vref_avg = sum(vref_samples) / len(vref_samples)
            print(f"    平均 Vref: {vref_avg:+.6f}V")
        ...
        # 条件満足度が90%以上なら折り返す
        return percentage >= threshold_percent, vref_avg, vref_samples
...
if __name__ == "__main__":
    ...
    sat_params = SaturationParams(
        ...
        vref_sat_initial=-0.1,         # Vref初期値[V]
