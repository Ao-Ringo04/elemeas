...
class MeasurementController:
    ...
    def __init__(self, dmm, drain_sum, gate_sum, filename: str = f"{formatted_time_and_name}=sat.txt",
                 save_columns: List[str] = ['Time', "GateI", "GateV", "DrainI", "Vref"],
                 enable_plot: bool = True):
        self.dmm = dmm
        self.drain_sum = drain_sum
        self.gate_sum = gate_sum
        self.plotter = None
        ...
        while self.measuring:
            ...
            if current_time >= next_measure_time:
                ...
                try:
                    ...
                    isd = self.drain_sum.read_current()     # Drain SUM: ドレイン電流 (A)
                    ig = self.gate_sum.read_current()       # Gate SUM: ゲート電流 (A)
    ...
    def _check_vref_reached_with_overshoot(self, target_vref: float, initial_vref: float, 
                                        ...
                                        tolerance_percent: float = 0.5,#比較サンプルの許容誤差 [%]
                                        threshold_percent: float = 90.0) -> Tuple[bool, float, List[float]]:
    
        num_samples = max(int(wait_time / data_interval), 1)#最低1stepで一回測定
    ...
    def measure_saturation(self, params: SaturationParams):
        ...
        self.drain_sum.set_voltage(params.vsd_sat)
        ...
        try:
            ...
            for cycle in range(params.cycle_num):
                ...
                while True:
                    step += 1
                    self.gate_sum.set_voltage(vg_current)
...
if __name__ == "__main__":
    ...
    drain_sum = Keithley2450Drain(rm=rm, 
                                  ip="192.168.1.102", 
                                  source_voltage=0.01, 
    ...
    gate_sum = Keithley2450Gate(rm=rm, 
                                ip="192.168.1.101", 
    ...
    try:
        ...
        controller = MeasurementController(dmm, drain_sum, gate_sum, enable_plot=True)
